...
main_controls <- c('oac_1', 'oac_2', 'oac_3', 'oac_4', 'oac_5', 'oac_6', # 'oac_7',
                   ...
                   'mean_age_adults', 'mean_age_children')
...
for (dv in dv_list){
  ...
  jpeg(paste('Longitudinal_Emissions/outputs/Regression/Regression_residuals/', dv, '.jpeg', sep=''), quality = 100)
  ...
  results_2 %>% 
    rbind(results_w) %>%
    write_csv(paste('Longitudinal_Emissions/outputs/Regression/Regression_', dv, '.csv', sep=''))
  ...
  data_w %>% 
    ...
    filter(age_dummy == 1) %>% 
    write_csv(paste('Longitudinal_Emissions/outputs/Data_W_from_R_', dv, '.csv', sep=''))
}
