...
all_data = results_og.loc[results_og['group'].isin(['All households', 'Other', 'Household with children']) == False]
for item in all_data[['group_var']].drop_duplicates()['group_var']:
    data = all_data.loc[all_data['group_var'] == item].drop('group_var', axis=1).fillna(0)
    data['product_2'] = data['product'] #+ ' ' + data['year_str']
    data = data.sort_values(['product_2', 'group'])
    data['ci_low'] = data['elasticity'] - data['ci_low']
    data['ci_up'] = data['ci_up'] - data['elasticity']
